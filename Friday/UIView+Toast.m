...
//general appearance for viewToHide Toast
static const CGFloat CSToastMaxWidthViewToHide            = 0.8;      // 80% of parent view width
static const CGFloat CSToastMaxHeightViewToHide           = 0.8;      // 80% of parent view height
//static const CGFloat CSToastHorizontalPaddingViewToHide   = 10.0;
static const CGFloat CSToastVerticalPaddingViewToHide     = 50.0;
static const CGFloat CSToastVerticalPaddingViewToHide2     = 10.0;
//static const CGFloat CSToastCornerRadiusViewToHide        = 10.0;
//static const CGFloat CSToastOpacityViewToHide             = 0.8;
static const CGFloat CSToastFontSizeViewToHide            = 14.0;
static const CGFloat CSToastMaxTitleLinesViewToHide       = 0;
static const CGFloat CSToastMaxMessageLinesViewToHide     = 0;
//static const NSTimeInterval CSToastFadeDurationViewToHide = 0.2;
...
// image view size
static const CGFloat CSToastImageViewWidth      = 70.0;
static const CGFloat CSToastImageViewHeight     = 70.0;
...
- (void)showToast:(UIView *)toast duration:(NSTimeInterval)duration {
    [self showToast:toast duration:duration position:CSToastActivityDefaultPosition];
}
...
- (UIView *)viewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image {
    ...
    if(image != nil) {
        ...
        imageWidth=MIN(CSToastImageViewWidth,image.size.width);
        imageHeight=MIN(CSToastImageViewHeight,image.size.height);
...
- (UIView *)viewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image viewToHide:(UIView *)viewToHide {
    // sanity
    if((message == nil) && (title == nil) && (image == nil)) return nil;
    ...
    UIView *wrapperView =[[UIView alloc] initWithFrame:CGRectMake(0,
                                                                  0,
    ...
    mask.frame = wrapperView.layer.bounds;
    CGRect biggerRect = CGRectMake(mask.frame.origin.x, mask.frame.origin.y, mask.frame.size.width, mask.frame.size.height);
    CGRect smallerRect = [viewToHide convertRect:viewToHide.bounds toView:wrapperView];
    ...
    if (title != nil) {
        titleLabel = [[UILabel alloc] init];
        titleLabel.numberOfLines = CSToastMaxTitleLinesViewToHide;
        titleLabel.font = [UIFont boldSystemFontOfSize:CSToastFontSizeViewToHide];
        titleLabel.textAlignment = NSTextAlignmentLeft;
        titleLabel.lineBreakMode = NSLineBreakByWordWrapping;
        titleLabel.textColor = [UIColor whiteColor];
        titleLabel.backgroundColor = [UIColor clearColor];
        ...
        // size the title label according to the length of the text
        CGSize maxSizeTitle = CGSizeMake((self.bounds.size.width * CSToastMaxWidthViewToHide) - imageWidth, self.bounds.size.height * CSToastMaxHeightViewToHide);
        CGSize expectedSizeTitle = [self sizeForString:title font:titleLabel.font constrainedToSize:maxSizeTitle lineBreakMode:titleLabel.lineBreakMode];
        titleLabel.frame = CGRectMake(0.0, 0.0, expectedSizeTitle.width, expectedSizeTitle.height);
    }
    ...
    if (message != nil) {
        messageLabel = [[UILabel alloc] init];
        messageLabel.numberOfLines = CSToastMaxMessageLinesViewToHide;
        messageLabel.font = [UIFont systemFontOfSize:CSToastFontSizeViewToHide];
        messageLabel.lineBreakMode = NSLineBreakByWordWrapping;
        messageLabel.textColor = [UIColor whiteColor];
        messageLabel.backgroundColor = [UIColor clearColor];
        messageLabel.alpha = 1.0;
        ...
        // size the message label according to the length of the text
        CGSize maxSizeMessage = CGSizeMake((self.bounds.size.width * CSToastMaxWidthViewToHide) - imageWidth, self.bounds.size.height * CSToastMaxHeightViewToHide);
        CGSize expectedSizeMessage = [self sizeForString:message font:messageLabel.font constrainedToSize:maxSizeMessage lineBreakMode:messageLabel.lineBreakMode];
        messageLabel.frame = CGRectMake(0.0, 0.0, expectedSizeMessage.width, expectedSizeMessage.height);
    }
    ...
    if(titleLabel != nil) {
        titleWidth = titleLabel.bounds.size.width;
        titleHeight = titleLabel.bounds.size.height;
        titleTop = CSToastVerticalPaddingViewToHide;
        titleLeft = (wrapperView.frame.size.width-titleWidth)/2;
    } else {
        titleWidth = titleHeight = titleTop = titleLeft = 0.0;
    }
    ...
    if(messageLabel != nil) {
        messageWidth = messageLabel.bounds.size.width;
        messageHeight = messageLabel.bounds.size.height;
        messageLeft = (wrapperView.frame.size.width-messageWidth)/2;;
        messageTop = titleTop + titleHeight + CSToastVerticalPaddingViewToHide2;
    } else {
        messageWidth = messageHeight = messageLeft = messageTop = 0.0;
    }
